...
describe("GET /v1/channels/:channelId", () => {
  it("should return channel detail with niche name and social accounts", async () => {
    ...
    expect(response.json().nicheName).toBe("Futebol")
    expect(response.json().socialAccounts).toEqual([])
  ...
  it("should reject without an access token", async () => {
    ...
    const response = await app.inject({ method: "GET", url: "/v1/channels/ghost" })
