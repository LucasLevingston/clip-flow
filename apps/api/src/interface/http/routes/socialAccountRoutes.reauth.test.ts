...
async function connectAccount(
  ...
  return { accountId: response.json().id as string, state }
}

describe("POST /v1/channels/:channelId/social-accounts/:id/reauth", () => {
  it("should reconnect the account", async () => {
    ...
    const { accountId, state } = await connectAccount(app, accessToken, channelId)

    const response = await app.inject({
      method: "POST",
      url: `/v1/channels/${channelId}/social-accounts/${accountId}/reauth`,
      headers: { authorization: `Bearer ${accessToken}` },
      payload: { code: "auth-code", state },
    })
  ...
  it("should reject without an access token", async () => {
    ...
    const response = await app.inject({
      method: "POST",
      url: "/v1/channels/ghost/social-accounts/ghost/reauth",
  ...
  it("should 404 when the account does not exist", async () => {
    ...
    const response = await app.inject({
      method: "POST",
      url: `/v1/channels/${channelId}/social-accounts/ghost/reauth`,
      headers: { authorization: `Bearer ${accessToken}` },
      payload: { code: "auth-code", state },
    })
