...
describe("GET /v1/channels/:channelId/social-accounts", () => {
  it("should list connected accounts after connecting YouTube", async () => {
    ...
    expect(response.json()).toEqual([
      expect.objectContaining({ platform: "YOUTUBE", status: "CONNECTED" }),
    ])
  ...
  it("should reject without an access token", async () => {
    ...
    const response = await app.inject({ method: "GET", url: "/v1/channels/ghost/social-accounts" })
  ...
  it("should 404 when the channel does not exist", async () => {
    ...
    const response = await app.inject({
      method: "GET",
      url: "/v1/channels/ghost/social-accounts",
      headers: { authorization: `Bearer ${accessToken}` },
    })
