import { Niche } from "../../../domain/catalog/entities/Niche"
import { buildTestServer } from "../../../test-utils/buildTestServer"

async function registerOwnerAndCreateChannel(app: ReturnType<typeof buildTestServer>["app"]) {
  const registerResponse = await app.inject({
    method: "POST",
    url: "/v1/auth/register",
    payload: { email: "owner@example.com", password: "Senha123", tenantName: "Studio" },
  })
  const accessToken = registerResponse.json().accessToken as string

  const createResponse = await app.inject({
    method: "POST",
    url: "/v1/channels",
    headers: { authorization: `Bearer ${accessToken}` },
    payload: {
      nicheId: "niche-1",
      name: "Meu Canal",
      language: "pt-BR",
      videosPerDay: 1,
      generationTime: "06:00",
      platforms: "SHORTS_ONLY",
      thumbnailEnabled: true,
    },
  })

  return { accessToken, channelId: createResponse.json().id as string }
}

function seedNiche(repo: ReturnType<typeof buildTestServer>["nicheRepository"]) {
  repo.seed(
    Niche.create({
      id: "niche-1",
      name: "Futebol",
      slug: "futebol",
      description: "desc",
      category: "Esportes",
      previewThumbnailUrl: null,
      status: "ACTIVE",
      createdAt: new Date(),
    }),
  )
}

async function connectAccount(
  app: ReturnType<typeof buildTestServer>["app"],
  accessToken: string,
  channelId: string,
) {
  const oauthUrlResponse = await app.inject({
    method: "GET",
    url: `/v1/channels/${channelId}/social-accounts/youtube/oauth-url`,
    headers: { authorization: `Bearer ${accessToken}` },
  })
  const state = new URL(oauthUrlResponse.json().url as string).searchParams.get("state") as string
  const response = await app.inject({
    method: "POST",
    url: `/v1/channels/${channelId}/social-accounts/youtube/oauth-callback`,
    headers: { authorization: `Bearer ${accessToken}` },
    payload: { code: "auth-code", state },
  })
  return response.json().id as string
}

describe("DELETE /v1/channels/:channelId/social-accounts/:id", () => {
  it("should disconnect the account and revert the channel to DRAFT", async () => {
    const { app, nicheRepository } = buildTestServer()
    seedNiche(nicheRepository)
    const { accessToken, channelId } = await registerOwnerAndCreateChannel(app)
    const accountId = await connectAccount(app, accessToken, channelId)

    const response = await app.inject({
      method: "DELETE",
      url: `/v1/channels/${channelId}/social-accounts/${accountId}`,
      headers: { authorization: `Bearer ${accessToken}` },
    })

    expect(response.statusCode).toBe(204)

    const channelResponse = await app.inject({
      method: "GET",
      url: `/v1/channels/${channelId}`,
      headers: { authorization: `Bearer ${accessToken}` },
    })
    expect(channelResponse.json().status).toBe("DRAFT")

    const listResponse = await app.inject({
      method: "GET",
      url: `/v1/channels/${channelId}/social-accounts`,
      headers: { authorization: `Bearer ${accessToken}` },
    })
    expect(listResponse.json()).toEqual([])

    await app.close()
  })

  it("should reject without an access token", async () => {
    const { app } = buildTestServer()

    const response = await app.inject({
      method: "DELETE",
      url: "/v1/channels/ghost/social-accounts/ghost",
    })

    expect(response.statusCode).toBe(401)

    await app.close()
  })

  it("should 404 when the account does not exist", async () => {
    const { app, nicheRepository } = buildTestServer()
    seedNiche(nicheRepository)
    const { accessToken, channelId } = await registerOwnerAndCreateChannel(app)

    const response = await app.inject({
      method: "DELETE",
      url: `/v1/channels/${channelId}/social-accounts/ghost`,
      headers: { authorization: `Bearer ${accessToken}` },
    })

    expect(response.statusCode).toBe(404)
    expect(response.json().error.code).toBe("SOCIAL_ACCOUNT_NOT_FOUND")

    await app.close()
  })
})
