...
describe("ClaudeAdapter", () => {
  it("should select a highlight from the tool_use response and record cost", async () => {
    const create = jest.fn().mockResolvedValue({
      usage: { input_tokens: 100, output_tokens: 20 },
      content: [
        {
          type: "tool_use",
          input: { startMs: 1_000, endMs: 21_000, transcriptSegmentIds: ["0"] },
        },
      ],
    })
    const costRecorder = new FakeAiCostRecorder()
    const adapter = new ClaudeAdapter(buildClient(create), costRecorder, "claude-model")
  ...
  it("should generate copy from the tool_use response", async () => {
    const create = jest.fn().mockResolvedValue({
    ...
    const adapter = new ClaudeAdapter(buildClient(create), new FakeAiCostRecorder(), "claude-model")
  ...
  it("should throw AiProviderInvalidResponseError when there is no tool_use block", async () => {
    const create = jest.fn().mockResolvedValue({
